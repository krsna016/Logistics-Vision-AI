...
class LocalLoadingSessionRepository implements LoadingSessionRepository {
  ...
  LoadingSession _map(db.LoadingSession data) {
    return LoadingSession(
      ...
      status: SessionStatus.values.firstWhere((e) => e.name == data.status, orElse: () => SessionStatus.started),
      ...
      metadata: data.metadata != null ? jsonDecode(data.metadata!) as Map<String, dynamic> : const {},
    );
  }
  ...
  Future<void> saveSession(LoadingSession session) async {
    await _db.transaction(() async {
      final exists = await (_db.select(_db.loadingSessions)..where((t) => t.id.equals(session.id))).getSingleOrNull();

      if (exists != null) {
        await (_db.update(_db.loadingSessions)..where((t) => t.id.equals(session.id))).write(db.LoadingSessionsCompanion(
        ...
        await _db.into(_db.loadingSessions).insert(db.LoadingSessionsCompanion.insert(
          id: session.id,
  ...
  Future<LoadingSession?> getSessionById(String id) async {
    try {
      final row = await (_db.select(_db.loadingSessions)..where((t) => t.id.equals(id))).getSingleOrNull();
  ...
  Future<LoadingSession?> getActiveSessionForTruck(String truckId) async {
    try {
      final query = _db.select(_db.loadingSessions)
        ..where((t) => t.truckId.equals(truckId) & t.status.equals(SessionStatus.started.name));
  ...
  Future<List<LoadingSession>> getSessionsByWarehouse(String warehouseId) async {
    try {
      final query = _db.select(_db.loadingSessions)..where((t) => t.warehouseId.equals(warehouseId) & t.isDeleted.equals(false));
  ...
  Future<LoadingSession?> recoverLastActiveSession() async {
    try {
      final query = _db.select(_db.loadingSessions)
        ..where((t) => t.status.equals(SessionStatus.started.name) | t.status.equals(SessionStatus.paused.name))
