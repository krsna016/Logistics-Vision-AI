...
class WagonListState {
  ...
  List<Wagon> get processedWagons {
    ...
    list.sort((a, b) {
      if (a.status == WagonStatus.loading && b.status != WagonStatus.loading) return -1;
      if (b.status == WagonStatus.loading && a.status != WagonStatus.loading) return 1;
...
class WagonListNotifier extends StateNotifier<WagonListState> {
  ...
  Future<void> refresh() async {
    ...
    try {
      ...
      state = state.copyWith(wagons: updatedList, isLoading: false, errorMessage: null);
    } catch (e) {
      state = state.copyWith(isLoading: false, errorMessage: 'Failed to read wagon records.');
    }
  }
  ...
  Future<String?> createWagon({
    ...
    final exists = await _repository.isWagonNumberExists(cleanNum);
    if (exists) return 'Wagon number already registered.';
  ...
  Future<void> updateWagonStatus(String id, WagonStatus newStatus) async {
    ...
    if (wagon != null) {
      final updated = wagon.copyWith(status: newStatus, updatedAt: DateTime.now());
