...
class LocalReportRepository implements ReportRepository {
  ...
  Future<List<ReportExport>> getExportHistory({int limit = 50}) async {
    final results = await (_db.select(_db.reportExports)
          ..orderBy([(t) => OrderingTerm(expression: t.exportedAt, mode: OrderingMode.desc)])
          ..limit(limit))
        .get();

    return results.map((r) => ReportExport(
          id: r.id,
          reportType: ReportType.values.firstWhere((e) => e.name == r.reportType, orElse: () => ReportType.wagon),
          exportFormat: ExportFormat.values.firstWhere((e) => e.name == r.exportType, orElse: () => ExportFormat.pdf),
          userId: r.userId,
          exportedAt: r.exportedAt,
          status: ExportStatus.values.firstWhere((e) => e.name == r.status, orElse: () => ExportStatus.failed),
          filePath: r.filePath,
          details: r.details,
        )).toList();
  }
