...
import '../../../../core/database/app_database.dart' hide User, AuditLog, DeviceSession;
...
class LocalAuthRepository implements AuthRepository {
  ...
  Future<User?> login(String employeeId, String password, {bool offline = false}) async {
    final user = await _authService.login(employeeId, password, offline: offline);
    if (user != null) {
      await logAction('Login', details: offline ? 'Offline mode' : 'Online mode', userId: user.id, userName: user.name, userRole: user.role);
    } else {
      await logAction('Login Failed', isSuccess: false, details: 'Invalid credentials or locked account for $employeeId', userId: 'unknown', userName: 'Unknown', userRole: Role.operator);
    }
    return user;
  }
  ...
  Future<void> logout() async {
    ...
    if (currentSession != null) {
      ...
      await logAction('Logout', userId: user?.id, userName: user?.name, userRole: user?.role);
    }
    await _authService.logout();
  }

  @override
  Future<Session?> getCurrentSession() async => _sessionManager.currentSession.value;
  ...
  Future<User?> getCurrentUser() async {
    ...
    final record = await (_db.select(_db.users)..where((t) => t.id.equals(session.userId))).getSingleOrNull();
    ...
    return User(
      ...
      name: record.name,
      role: Role.values.firstWhere((r) => r.toString().split('.').last == record.role, orElse: () => Role.operator),
  ...
  Future<void> lockSession() async {
    ...
    final user = await getCurrentUser();
    await logAction('Session Locked', userId: user?.id, userName: user?.name, userRole: user?.role);
  }
  ...
  Future<bool> unlockSession(String pinOrPassword) async {
    ...
    if (isValid) {
      ...
      await logAction('Session Unlocked', userId: user.id, userName: user.name, userRole: user.role);
      return true;
    }
    
    await logAction('Session Unlock Failed', isSuccess: false, userId: user.id, userName: user.name, userRole: user.role);
    return false;
  }
  ...
  Future<List<AuditLog>> getAuditLogs() async {
    ...
    return records.map((r) => AuditLog(
      id: r.id,
      userId: r.userId,
      userName: 'Unknown', // Would join with Users table in a real query
  ...
  Future<List<User>> getAllUsers() async {
    ...
    return records.map((r) => User(
      id: r.id,
      employeeId: r.employeeId,
      name: r.name,
      role: Role.values.firstWhere((role) => role.toString().split('.').last == r.role, orElse: () => Role.operator),
  ...
  Future<void> createUser(User user) async {
    ...
    await logAction('Created User', details: 'Added ${user.employeeId}', userId: currentUser?.id, userName: currentUser?.name, userRole: currentUser?.role);
  }
  ...
  Future<void> toggleUserStatus(String userId, bool isActive) async {
    ...
    final currentUser = await getCurrentUser();
    await logAction(isActive ? 'Enabled User' : 'Disabled User', details: 'User ID: $userId', userId: currentUser?.id, userName: currentUser?.name, userRole: currentUser?.role);
  }

  @override
  Future<List<DeviceSession>> getRegisteredDevices() async {
    final records = await _db.select(_db.deviceSessions).get();
    return records.map((r) => DeviceSession(
      id: r.id,
