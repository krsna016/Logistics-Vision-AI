...
class OfflineAuthenticationImpl implements OfflineAuthentication {
  ...
  const OfflineAuthenticationImpl(this._db, this._credentialStorage, this._passwordHasher);
  ...
  Future<User?> authenticate(String employeeId, String password) async {
    ...
    final storedHash = await _credentialStorage.getHashedPassword(normalizedEmployeeId);
    ...
    return User(
      ...
      name: user.name,
      role: Role.values.firstWhere((r) => r.toString().split('.').last == user.role, orElse: () => Role.operator),
