...
def test_user_creation_accepts_only_the_two_supported_roles():
    valid = UserCreate(employee_id="EMP-1", name="Supervisor", role="Supervisor", password="a-secure-passphrase")
    assert valid.role == "Supervisor"
    admin = UserCreate(employee_id="ADM-1", name="Admin", role="Administrator", password="another-secure-passphrase")
    ...
    with pytest.raises(ValueError):
        UserCreate(employee_id="EMP-1", name="User", role="Supervisor", password="too-short")
