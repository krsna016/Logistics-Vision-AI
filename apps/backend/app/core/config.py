...
class Settings(BaseSettings):
    ...
    RESET_ADMIN_PASSWORD: bool = Field(
        default=False, validation_alias="RESET_ADMIN_PASSWORD"
    )
    ADMIN_CORS_ORIGINS: str = Field(
        default="http://localhost:5173", validation_alias="ADMIN_CORS_ORIGINS"
    )
    TRUSTED_HOSTS: str = Field(
        default="localhost,127.0.0.1,testserver", validation_alias="TRUSTED_HOSTS"
    )
    AUTO_CREATE_SCHEMA: bool = Field(
        default=True, validation_alias="AUTO_CREATE_SCHEMA"
    )

    # SQLite for development, Postgres for prod
    DATABASE_URL: str = Field(
        default="sqlite+aiosqlite:///./sql_app.db", validation_alias="DATABASE_URL"
    )
    ...
    def validate_runtime_secrets(self) -> None:
        ...
        if (
            ...
            or self.ACCESS_TOKEN_EXPIRE_MINUTES > 1440
        ):
        ...
        if self.is_production:
            ...
            if self.AUTO_CREATE_SCHEMA:
                raise RuntimeError(
                    "AUTO_CREATE_SCHEMA must be false in production; run migrations before startup"
                )
