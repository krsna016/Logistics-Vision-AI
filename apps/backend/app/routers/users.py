...
async def create_user(user_in: UserCreate, db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)):
...
async def update_user(employee_id: str, user_in: UserUpdate, db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)):
    result = await db.execute(select(User).where(User.employee_id == employee_id))
    user = result.scalars().first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def disable_user(employee_id: str, db: AsyncSession = Depends(get_db), actor: User = Depends(require_admin)):
...
async def activate_user(employee_id: str, db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)):
    result = await db.execute(select(User).where(User.employee_id == employee_id))
    user = result.scalars().first()
    if not user:
        raise HTTPException(status_code=404, detail="User not found")
...
async def hard_delete_user(employee_id: str, db: AsyncSession = Depends(get_db), actor: User = Depends(require_admin)):
    ...
    await db.execute(delete(LocationPing).where(LocationPing.employee_id == employee_id))
    await db.execute(delete(LocationSession).where(LocationSession.employee_id == employee_id))
