...
## Model files are intentionally excluded

Git does not include trained `.pt` checkpoints. PyTorch models can be large, private, and executable. After starting the app, add a trusted YOLO detection or segmentation checkpoint through **Model library → Add model**. Uploaded models are stored locally in `models/` and remain available after restart.
